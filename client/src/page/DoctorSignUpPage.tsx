  ...
  const [phone, setPhone] = useState<string|null>(null);
    ...
    const formikSendOtp = useFormik<FormValuesSendOtp>({
      ...
      onSubmit: async values => {
       ...
       const response = await axios.post(`${BASE_URL}api/v1/register_with_phone`, { phone: values.phoneNumber, name:values.username, role: "doctor" });
            ...
            <form className="py-1"  onSubmit={formikSendOtp.handleSubmit}>
              ...
              <div className="text-center py-4 text-xs">
                <span className="text-gray-500 block">
                  Already Registered?{' '}
                  <Link to="/doctor/login" replace={true} className="text-blue-700 hover:underline cursor-pointer">
                    Login now
                  </Link>
                </span>
              </div>
