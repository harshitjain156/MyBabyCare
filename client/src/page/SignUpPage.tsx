...
export default function SignUpPage() {
  ...
  const [phone, setPhone] = useState<string|null>(null);
    ...
    const otpInputsRef = useRef<(HTMLInputElement | null)[]>([null, null, null, null]);
    ...
    const formikSendOtp = useFormik<FormValuesSendOtp>({
      ...
      onSubmit: async values => {
       ...
       const response = await axios.post(`${BASE_URL}api/v1/register_with_phone`, { phone: values.phoneNumber, name:values.username, role: "user" });
    ...
    const formikVerifyOtp = useFormik<FormValuesVerifyOtp>({
      ...
      onSubmit: async values => {
        try {
          ...
          const response = await axios.post(`${BASE_URL}api/v1/verify`, { otp: `${values.otp0}${values.otp1}${values.otp2}${values.otp3}`, userId });
    ...
    const handleResendOTP = async () => {
      try {
        ...
        const response = await axios.post('http://localhost:8000/api/v1/login_with_phone', { phone });
      ...
      <div className="w-full flex justify-center items-center">
        <div className="w-full md:max-w-md py-4 border-2">
          ...
          {!showOtpBox ? (
            <form className="py-1"  onSubmit={formikSendOtp.handleSubmit}>
              ...
              <input
                  ...
                  className={`appearance-none block sm:w-2/4 bg-gray-200 text-gray-700 border ${
                    formikSendOtp.errors.username ? 'border-red-500' : 'border-gray-200'
              ...
              <div className="text-center py-4 text-xs">
                <span className="text-gray-500 block">
                  Already Registered?{' '}
                  <Link to="/user/login" replace={true} className="text-blue-700 hover:underline cursor-pointer">
                    Login now
                  </Link>
                </span>
              </div>
            ...
            <form className="py-1" ref={formRef}  onSubmit={formikVerifyOtp.handleSubmit}>
              <div className="textbox flex flex-col items-center gap-8 w-full">
              ...
              {[0,1,2,3].map(index => (
                      <input
                        ...
                        className={`appearance-none block w-1/4 bg-gray-200 text-gray-700 border ${
                          formikVerifyOtp.errors[`otp${index}` as keyof typeof formikVerifyOtp.errors]
                            ? 'border-red-500'
                            : 'border-gray-200'
                        } rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white`}
                      />
              ...
              <div className="text-center py-4 text-xs">
                ...
                {seconds === 0 && (
                  <span className="text-blue-700 hover:underline cursor-pointer mb-2" onClick={handleResendOTP}>
                    Resend OTP
                  </span>
                )}
                 <span className="text-gray-500 block">
