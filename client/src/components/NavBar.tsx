...
interface NavBarProps {
  ...
  setHideNotificationContainer: (isOpen: boolean) => void;
  setSidebarOpen: (isOpen: boolean) => void;
  sidebarOpen: boolean;
}

const NavBar: React.FC<NavBarProps> = ({
  setHideChatContainer,
  ...
  return (
    <nav className="bg-primary py-4 inline-block  w-screen fixed top-0 left-0 shadow-lg z-40">
      <div className="w-full mx-auto px-4   flex justify-between items-center">
        ...
        <div className="text-lg text-white font-semibold flex justify-start items-center">
          {userData &&
            pathname.includes(userData?.role.toLowerCase()) &&
        ...
        <ul className="flex item-center gap-6 ">
          {userData &&
            ...
            !pathname.includes("signup") && (
              <li
                onClick={() => {
                  setHideNotificationContainer(false);
                }}
                className="text-white py-2 px-4 relative rounded-full bg-secondary cursor-pointer flex gap-1 justify-center items-center  hover:bg-secondary-dark"
              >
                <svg
          ...
          {userData &&
            ...
            !pathname.includes("signup") && (
              <li
                onClick={() => {
                  setHideChatContainer(false);
                }}
                className="text-white py-2 px-4 rounded-full bg-secondary cursor-pointer flex gap-1 justify-center items-center  hover:bg-secondary-dark"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
              ...
              <li>
                <span className="w-full flex items-center rounded-full overflow-hidden">
                  <img
                    ...
                    className="h-11 w-11  object-cover opacity-75"
                  />
                </span>
              </li>
            )}
          {/* <li className="bg-secondary text-white py-2 px-8 rounded-full cursor-pointer hover:bg-secondary-dark"><Link to="/login">Sign In</Link></li> */}
          {pathname.includes("user") && pathname.includes("login") && (
            <li className="bg-white text-gray-500 py-2 px-8 rounded-full cursor-pointer hover:bg-gray-200">
